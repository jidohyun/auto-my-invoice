defmodule AutoMyInvoice.FoundationTest do
  use ExUnit.Case

  # F-2: Oban 큐 초기화
  describe "Oban configuration" do
    test "all queues are configured" do
      config = Application.fetch_env!(:auto_my_invoice, Oban)
  ...
  describe "PubSub broadcast/subscribe" do
    test "can subscribe and receive messages" do
      ...
      Phoenix.PubSub.broadcast(AutoMyInvoice.PubSub, topic, {:test_event, "hello"})
    ...
    test "uses PubSubTopics for topic names" do
      ...
      Phoenix.PubSub.subscribe(AutoMyInvoice.PubSub, topic)
      Phoenix.PubSub.broadcast(AutoMyInvoice.PubSub, topic, {:invoice_updated, %{id: "test-id"}})
  ...
  describe "Cachex cache" do
    test "can put and get values" do
      Cachex.put(:auto_my_invoice_cache, "test_key", "test_value")
      assert {:ok, "test_value"} = Cachex.get(:auto_my_invoice_cache, "test_key")
    end

    test "returns nil for missing keys" do
      assert {:ok, nil} = Cachex.get(:auto_my_invoice_cache, "nonexistent_key")
    end
  end
end
